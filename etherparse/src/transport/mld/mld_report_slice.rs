...
/// Zero-copy slice of an MLDv1 "Multicast Listener Report" (ICMPv6 type
/// `131`, exactly 24 octets long).
///
/// Defined in
...
/// ```
///
/// Note that the "Maximum Response Delay" field is set to zero in report
/// messages and ignored by receivers.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MldReportSlice<'a> {
    slice: &'a [u8],
}

impl<'a> MldReportSlice<'a> {
    ...
    #[inline]
    pub(crate) unsafe fn from_slice_unchecked(slice: &'a [u8]) -> MldReportSlice<'a> {
        debug_assert!(slice.len() >= MldV1Header::LEN);
        MldReportSlice { slice }
    }
    ...
    /// The "Maximum Response Delay" field (sent as zero in reports).
    #[inline]
    ...
    /// The multicast address the sender is listening to.
    #[inline]
