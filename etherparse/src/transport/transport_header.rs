...
impl TransportHeader {
    ...
    /// Returns Result::Some containing the IGMPv1 header if self has the value Igmpv1.
    /// Otherwise None is returned.
    pub fn igmpv1(self) -> Option<Igmpv1Header> {
        use crate::TransportHeader::*;
        if let Igmpv1(value) = self {
            Some(value)
        } else {
            None
        }
    ...
    /// Returns Result::Some containing a mutable reference to the IGMPv1 header if self has the value Igmpv1.
    /// Otherwise None is returned.
    pub fn mut_igmpv1(&mut self) -> Option<&mut Igmpv1Header> {
        use crate::TransportHeader::*;
        if let Igmpv1(value) = self {
            Some(value)
        } else {
    ...
    pub fn update_checksum_ipv6(
        ...
        match self {
            ...
            Igmpv1(header) => header.update_checksum(),
            Icmpv6(header) => {
    ...
    pub fn write<T: std::io::Write + Sized>(&self, writer: &mut T) -> Result<(), std::io::Error> {
        ...
        match self {
            ...
            Igmpv1(value) => value.write(writer),
