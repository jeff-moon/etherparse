...
/// Zero-copy slice of an IGMPv1 "Membership Report" (type `0x12`).
///
/// ```text
/// 0                   1                   2                   3
...
pub struct MembershipReportV1Slice<'a> {
    slice: &'a [u8],
}

impl<'a> MembershipReportV1Slice<'a> {
    /// Creates a slice from bytes without checking the length.
    ///
    /// # Safety
    ///
    /// The caller must guarantee that `slice` is at least
    /// [`MembershipReportV1Type::LEN`] (8) bytes long.
    #[inline]
    pub(crate) unsafe fn from_slice_unchecked(slice: &'a [u8]) -> MembershipReportV1Slice<'a> {
        debug_assert!(slice.len() >= MembershipReportV1Type::LEN);
        MembershipReportV1Slice { slice }
    }
    ...
    /// Decodes the fixed fields into an owned [`MembershipReportV1Type`].
    #[inline]
    ...
    pub fn payload(&self) -> &'a [u8] {
        ...
        unsafe {
            core::slice::from_raw_parts(
                self.slice.as_ptr().add(MembershipReportV1Type::LEN),
                self.slice.len() - MembershipReportV1Type::LEN,
            )
        }
    }
